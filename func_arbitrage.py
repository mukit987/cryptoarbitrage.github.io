...
def append_to_html(result, execution_time, start_time_str, end_time_str, filename):
    ...
    # Find the index where the new row should be inserted (just after <tbody>)
    insert_index = next((index for index, line in enumerate(lines) if '<tbody>' in line), len(lines))

    # Create the new row as a string
    new_row = f"""
            <tr>
                <td>{result['profit_loss']:.6f}</td>
                <td>{result['real_rate_perc']:.6f}%</td>
             ...
             <tr>
                <td colspan="11">
                    ...
                    Gross Profit/Loss: {result['profit_loss']:.6f} USDT<br>
                    Total Trading Fee (0.09% per trade): 0.27 USDT<br>
                    Net Profit/Loss (after fees): {result['profit_loss'] - 0.27:.6f} USDT
                </td>
            </tr>
    """

    # Insert the new row into the list of lines
